...
//	bt_mesh_prov_enable(BT_MESH_PROV_ADV | BT_MESH_PROV_GATT);
	...
	bt_mesh_shell_target_ctx.dst = 0xA;
	...
	err = bt_mesh_provision(net_key, 0, 0, 0, 0x0001, dev_key);
	if (err) {
		printk("Unable to provision: %d\n", err);
	}

	const uint8_t app_key[16];
	status = bt_mesh_app_key_add(0, 0, app_key);
	if (status) {
		printk("Unable to add AppKey: %d\n", status);
	}
