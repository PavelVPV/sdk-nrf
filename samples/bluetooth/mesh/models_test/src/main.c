...
#define LOG_MODEL(mod, fmt, ...) printk("[%04x:%04x]: " fmt,\
					bt_mesh_model_elem(mod)->rt->addr,\
					mod->id, __VA_ARGS__)
...
{
	LOG_MODEL(cli->model, "lc_prop: %d %s\n", id, bt_mesh_sensor_ch_str(value));
}
...
static struct bt_mesh_lightness_srv lightness_srv_xyl =
	BT_MESH_LIGHTNESS_SRV_INIT(&lightness_srv_handlers);
static struct bt_mesh_light_xyl_srv light_xyl_srv =
	BT_MESH_LIGHT_XYL_SRV_INIT(&lightness_srv_xyl, &xyl_srv_handelrs);
		  ...
		  struct bt_mesh_light_hue_status *rsp)
{
	...
	int32_t rt = remaining_time_get(set->transition);

	rsp->current = rt ? hue : set->lvl;
	rsp->target = set->lvl;
	rsp->remaining_time = rt;
...
static struct bt_mesh_lightness_srv lightness_srv_hsl =
	BT_MESH_LIGHTNESS_SRV_INIT(&lightness_srv_handlers);
static struct bt_mesh_light_hsl_srv light_hsl_srv =
	BT_MESH_LIGHT_HSL_SRV_INIT(&lightness_srv_hsl, &hue_srv_handlers, &sat_srv_handlers);
...
static int motion_get(struct bt_mesh_sensor_srv *srv,
...
static struct bt_mesh_sensor_value motion_threshold;
...
{
#if 0
	printk("motion_threshold_get: sen:[%d], set:[%d], val:[%s], wr:[%d]\n",
	       sensor->type->id, setting->type->id,
	       bt_mesh_sensor_ch_str(motion_threshold),
	       setting->set != NULL);
#else
	__ASSERT(0, "Not implemented");
#endif
...
static int motion_threshold_set(struct bt_mesh_sensor_srv *srv,
				...
				const struct bt_mesh_sensor_value *value)
{
	printk("motion_threshold_set: sen:[%d], set:[%d], val:[%s], wr:[%d]\n",
	       sensor->type->id, setting->type->id, bt_mesh_sensor_ch_str(value), setting->set != NULL);
			  ...
			  const struct bt_mesh_sensor_value *value)
{
	...
	for (size_t i = 0; i < sensor->channel_count; i++) {
		LOG_MODEL(cli->model, "\t[%d]: %s\n", i, bt_mesh_sensor_ch_str(value));
	}
}
...
/* Server models */
	...
	BT_MESH_ELEM(
		3, BT_MESH_MODEL_LIST(
			BT_MESH_MODEL_LIGHTNESS_SRV(&lightness_srv_xyl)),
		BT_MESH_MODEL_NONE),
	BT_MESH_ELEM(
	...
	BT_MESH_ELEM(
		10, BT_MESH_MODEL_LIST(
			BT_MESH_MODEL_LIGHTNESS_SRV(&lightness_srv_hsl),
...
#if 0
	struct bt_mesh_sensor_value setting_value;
	err = bt_mesh_sensor_value_from_micro(bt_mesh_motion_threshold.channels[0].format,
					      &setting_value);
	if (err) {
		shell_print(app_shell, "Failed to convert value to micro: %d", err);
		return err;
	}
...
static int cmd_krp_get(const struct shell *shell, size_t argc, char *argv[])
{
#if 0
	uint8_t status;
	uint8_t phase = 0;
	...
	err = bt_mesh_cfg_krp_get(BT_MESH_KEY_PRIMARY, addr, BT_MESH_KEY_PRIMARY, &status, &phase);
	shell_print(app_shell, "KeyRefresh Get, err: %d, status: %d, phase: %d", err, status, phase);
#else
	shell_print(app_shell, "Not supported");
#endif

	return 0;
}

static int cmd_krp_set(const struct shell *shell, size_t argc, char *argv[])
{
#if 0
	uint8_t status;
	uint8_t phase;
	...
	err = bt_mesh_cfg_krp_set(BT_MESH_KEY_PRIMARY, addr, BT_MESH_KEY_PRIMARY, phase, &status,
				  &phase);
	shell_print(app_shell, "KeyRefresh Set, err: %d, status: %d, phase: %d", err, status, phase);
#else
	shell_print(app_shell, "Not supported");
#endif
...
static int cmd_detkey_get(const struct shell *shell, size_t argc, char *argv[])
{
	uint8_t dev_key[33];

	bin2hex((uint8_t*)&bt_mesh.dev_key, 16, dev_key, sizeof(dev_key));
	dev_key[32] = 0;
	shell_print(app_shell, "DevKey: %s", dev_key);

	return 0;
}

static uint8_t net_key[16] = { 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
			       0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f };

static int cmd_netkey_set(const struct shell *shell, size_t argc, char *argv[])
{
	size_t len;
...
static int cmd_netkey_update(const struct shell *shell, size_t argc, char *argv[])
{
#if 0
	uint8_t status;
	uint16_t addr;
	...
	err = bt_mesh_cfg_cli_net_key_update(BT_MESH_KEY_PRIMARY, addr, BT_MESH_KEY_PRIMARY, net_key,
					 &status);
	shell_print(app_shell, "NetKey Update, err: %d, status: %d", err, status);
#else
	shell_print(app_shell, "Not supported");
#endif
...
SHELL_STATIC_SUBCMD_SET_CREATE(app_cmds,
	...
	SHELL_CMD_ARG(krp_get, NULL, "Get KeyRefresh phase", cmd_krp_get, 1, 0),
	SHELL_CMD_ARG(krp_set, NULL, "Set KeyRefresh phase: <phase>", cmd_krp_set, 2, 0),
	SHELL_CMD_ARG(detkey_get, NULL, "Get DetKey", cmd_detkey_get, 1, 0),
	SHELL_CMD_ARG(netkey_set, NULL, "Set new primary NetKey", cmd_netkey_set, 2, 0),
	SHELL_CMD_ARG(netkey_update, NULL, "Update primary NetKey", cmd_netkey_update, 2, 0),
