...
LOG_MODULE_REGISTER(dfu_target_dist, CONFIG_DFU_TARGET_LOG_LEVEL);
...
int dfu_target_dist_offset_get(size_t *out)
{
	int err = 0;
...
{
	/* Not supported */
	/* Could potentially start Bluetoot mesh DFU if distributor is prepared. */
	return -ENOTSUP;
}
